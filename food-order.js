...
// For spa-nav hook
window.renderFoodOrder = renderFoodOrder;

// Static data for meals/gyms
const MEALS = [
...
function renderFoodOrder() {
  ...
  let gymOptions = GYMS.map(g =>
    `<option value="${g.code}">${g.name}</option>`).join('');
  foodSection.innerHTML = `
    <form id="food-order-form" autocomplete="off">
      ...
      <input type="date" id="collection-date" required min="${new Date().toISOString().split('T')[0]}"><br>
  ...
  document.getElementById('food-order-form').addEventListener('submit', async function (e) {
    ...
    try {
      await addDoc(collection(db, "food_orders"), {
        userId: user.uid, meal: meal.name, gym: gym.name, desc: meal.desc,
        date, timestamp: serverTimestamp()
      });
      messageDiv.textContent =
        `Order placed! Collect from ${gym.name} on ${date}.`;
      this.reset();
  ...
  // Show order history whenever form shown
  loadFoodOrders();
}
